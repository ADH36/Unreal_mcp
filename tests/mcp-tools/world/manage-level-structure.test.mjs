...
// === VOLUME ACTIONS ===
{
  ...
  testCases.push(
    ...
    { scenario: 'CREATE: create_nav_mesh_bounds_volume', toolName: 'manage_level_structure', arguments: {"action": "create_nav_mesh_bounds_volume", "volumeName": "Testnav_mesh_bounds_volume", "extent": {"x": 500, "y": 500, "z": 200}}, expected: 'success|already exists', assertions: [{ path: 'structuredContent.boundsExtentX', predicate: value => Number(value) > 0, label: 'X brush bounds are nonzero' }, { path: 'structuredContent.boundsExtentY', predicate: value => Number(value) > 0, label: 'Y brush bounds are nonzero' }, { path: 'structuredContent.boundsExtentZ', predicate: value => Number(value) > 0, label: 'Z brush bounds are nonzero' }] },
